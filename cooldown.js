...
async function shouldSkipCycle(brainUrl, brainKey) {
  ...
  var bh = { apikey: brainKey, Authorization: 'Bearer ' + brainKey, 'Accept-Profile': 'abacia_core' };
  try {
    var ctrl = new AbortController();
    setTimeout(function() { ctrl.abort(); }, 5000);
    var r = await fetch(
      brainUrl + '/rest/v1/aibe_brain?stamp_type=eq.SPAN_DISPATCH_COOLDOWN&source=like.eanew.cooldown*&order=created_at.desc&limit=1&select=content',
...
async function stampCooldown(brainUrl, brainKey, hamUid) {
  ...
  var bh = { apikey: brainKey, Authorization: 'Bearer ' + brainKey,
    'Accept-Profile': 'abacia_core', 'Content-Profile': 'abacia_core',
  ...
  try {
    await fetch(brainUrl + '/rest/v1/aibe_brain',
